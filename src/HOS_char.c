...
//See documentation
int stringcompare(char sights[], char goal[], int start, int length)
{
  int comp_ptr = 0;
  char return_value = 0;

  if( length ) //if the length is a number
  {
	  return_value = 1;  //set to zero if one case of difference is found
	  for(comp_ptr = start; comp_ptr < (start + length); comp_ptr++)
	  {
		  if(sights[comp_ptr] != goal[comp_ptr - start])
		  {
			  return_value = 0;
		  }
	  }
  }
  else if( !start && !length ) //if neither are numbers
  {
	  return_value = 1;  //set to zero if one case of difference is found
	  for(comp_ptr = start; goal[comp_ptr + 1] != 0x00; comp_ptr++) //seeks for the next char to be null
	  {
...
//
//  (int start) should be a pointer to the first number.
//
//Will stop looking on discovery of a line control char, null, space, or after 6 chars.
int search_int(char sights[], int start)
{
  int i;
  int number = 0;
  int last_char;
  ...
  for(i = 0; i <= 5; i++)
  {
    if((sights[start + i] == 0x20)||(sights[start + i] == 0x0A)||(sights[start + i] == 0x0D))
    {
      last_char = start + i - 1;  //point to last number
      i = 6;  //Bust on outta here!
    }
  }

  //now work backwards building the number
  for(i = last_char; i >= start; i--)
  {
...
//   null-terminated string.  It takes an argument of the starting address to
